...
python3 -c "import boto3, matplotlib, pydantic_settings, dropbox" 2>/dev/null || {
    echo "⚠️  Installing missing dependencies (boto3, matplotlib, pydantic-settings, dropbox)..."
    pip install -q boto3 matplotlib pydantic-settings dropbox || echo "⚠️  Some dependencies may be missing, continuing anyway..."
}
