#!/usr/bin/env python3
"""
Run Engine Training on Top N Coins

This script:
1. Fetches top N coins from Binance
2. Uses cached candle data (or downloads if missing)
3. Builds features
4. Trains models
5. Reports how many days of data were used
6. Uploads models to Dropbox

Usage:
    python scripts/run_engine_top_n.py --dropbox-token YOUR_TOKEN --top 20
"""
...
def get_top_coins_from_binance(top_n: int = 20, min_volume_usdt: float = 1_000_000) -> List[str]:
    """Get top N coins from Binance by 24h volume."""
    try:
        import ccxt
        ...
        print(f"   📊 Fetching 24h volume data...")
        tickers = exchange.fetch_tickers()
        
        # Filter and sort by volume
        pairs_with_volume = []
        stablecoins = {'USDT', 'BUSD', 'USDC', 'TUSD', 'USDD', 'DAI', 'FDUSD', 'PAXG', 'WBTC'}
        
        for symbol in usdt_pairs:
            if symbol in tickers:
                ticker = tickers[symbol]
                volume = ticker.get('quoteVolume', 0) or 0  # USDT volume
                
                # Skip stablecoins and inactive markets
                base = symbol.split('/')[0]
        ...
        # Return top N
        top_coins = [symbol for symbol, _ in pairs_with_volume[:top_n]]
        print(f"   ✅ Selected top {len(top_coins)} coins by volume")
        return top_coins
    
    except Exception as e:
        print(f"⚠️  Error fetching coins from Binance: {e}")
        import traceback
        traceback.print_exc()
        print(f"   Using default top {top_n} coins...")
        # Fallback to known top coins
        default_coins = [
...
def main():
    parser = argparse.ArgumentParser(description="Run engine training on top N coins")
    ...
    parser.add_argument("--top", type=int, default=20, help="Number of top coins to train (default: 20)")
    ...
    print(f"✅ Selected coins: {', '.join(symbols)}")
    print()
    
    print("=" * 70)
    print(f"🚀 ENGINE TRAINING ON TOP {len(symbols)} COINS")
    print("=" * 70)
    ...
    # Check which symbols need data download
    print("=" * 70)
    ...
    for symbol in symbols:
        ...
        if df is None or df.empty:
            ...
            print(f"   ⚠️  {symbol}: No cached data, will download")
        else:
            symbols_ready.append(symbol)
            print(f"   ✅ {symbol}: Cached data available")
    
    print()
    print(f"✅ Ready to train: {len(symbols_ready)} coins")
    print(f"📥 Need download: {len(symbols_needing_download)} coins")
    print()
    
    # Download missing data if needed
    if symbols_needing_download and args.dropbox_token:
        print("=" * 70)
        print(f"📥 DOWNLOADING DATA FOR {len(symbols_needing_download)} COINS")
        print("=" * 70)
        print()
        ...
        try:
            ...
            for idx, symbol in enumerate(symbols_needing_download, 1):
                print(f"[{idx}/{len(symbols_needing_download)}] 📥 Downloading {symbol}...")
                try:
                    download_and_upload(
                        symbols=[symbol],
                        days=lookback_days + 50,  # Download a bit more for buffer
                        exchange_id="binance",
            ...
            print("   Continuing with available cached data...")
            print()
    ...
    for idx, symbol in enumerate(symbols, 1):
        ...
        try:
            ...
            # Calculate actual days used and limit to lookback_days
    ...
    print(f"📊 Training completed on {successful} out of {len(symbols)} coins")
    print()
