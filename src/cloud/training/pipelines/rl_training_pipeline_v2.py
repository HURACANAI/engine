"""
Enhanced RL Training Pipeline with V2 Data Pipeline

Extends the existing RLTrainingPipeline with production-grade data preparation:
- Triple-barrier labeling (no lookahead bias)
- Realistic cost modeling (TCA)
- Meta-labeling (profitable after costs)
- Recency weighting (exponential decay)
- Walk-forward validation (optional)

This is a DROP-IN replacement for RLTrainingPipeline with better data quality.
"""
...
class RLTrainingPipelineV2:
    """
    Enhanced RL training pipeline with V2 data preparation.

    Key improvements over original:
    1. Triple-barrier labeling (prevents lookahead bias)
    2. Realistic cost modeling (fees + spread + slippage)
    3. Meta-labeling (filters to profitable-after-costs trades)
    4. Recency weighting (recent data weighted higher)
    ...
    def __init__(
        ...
        v2_config: V2PipelineConfig | None = None,
        trading_mode: str = 'scalp'
    ):
        """
        Initialize enhanced RL training pipeline.

        Args:
            settings: Engine settings
            dsn: Database connection string
            use_v2_pipeline: Enable V2 data pipeline (recommended)
            v2_config: Custom V2 config (optional)
            trading_mode: 'scalp' or 'runner'
        """
        self.settings = settings
        self.dsn = dsn
        self.use_v2_pipeline = use_v2_pipeline
        self.trading_mode = trading_mode

        # Initialize V2 pipeline adapter
        if use_v2_pipeline:
            if v2_config:
                self.v2_adapter = V2PipelineAdapter(config=v2_config)
            elif trading_mode == 'scalp':
                self.v2_adapter = create_v2_scalp_adapter()
            else:
                self.v2_adapter = create_v2_runner_adapter()

            logger.info(
                "v2_pipeline_enabled",
                mode=trading_mode,
                data_quality=self.v2_adapter.config.enable_data_quality,
                meta_labeling=self.v2_adapter.config.enable_meta_labeling,
                recency_weighting=self.v2_adapter.config.enable_recency_weighting
            )
        else:
            self.v2_adapter = None
            logger.info("v2_pipeline_disabled", message="Using legacy labeling")
        ...
        logger.info("rl_training_pipeline_v2_initialized")
    ...
    def train_on_symbol(
        ...
        V2 enhancements:
        - Data quality pipeline (dedup, outliers, gaps)
        ...
        Args:
            ...
            lookback_days: How far back to train (365 = 1 year)

        Returns:
            Training metrics and statistics
        """
        logger.info("training_start_v2", symbol=symbol, lookback_days=lookback_days, v2_enabled=self.use_v2_pipeline)
        ...
        # 2. V2 Data Pipeline (if enabled)
        v2_metrics = {}
        if self.use_v2_pipeline and self.v2_adapter:
            logger.info("running_v2_data_pipeline", symbol=symbol)

            # Process through V2 pipeline
            labeled_trades, weights = self.v2_adapter.process(
                data=historical_data,
                symbol=symbol
            )

            if not labeled_trades:
                logger.warning("v2_pipeline_no_trades", symbol=symbol)
                # Fall back to legacy pipeline
            else:
                # Convert to legacy format for compatibility with ShadowTrader
                # (This is temporary - eventually ShadowTrader will use V2 directly)
                legacy_df = self.v2_adapter.convert_to_legacy_format(
                    labeled_trades=labeled_trades,
                    weights=weights
                )

                v2_metrics = {
                    'v2_total_labels': len(labeled_trades),
                    'v2_profitable_labels': sum(1 for t in labeled_trades if t.meta_label == 1),
                    'v2_profitable_pct': sum(1 for t in labeled_trades if t.meta_label == 1) / len(labeled_trades) * 100 if labeled_trades else 0,
                    'v2_avg_net_pnl_bps': sum(t.pnl_net_bps for t in labeled_trades) / len(labeled_trades) if labeled_trades else 0,
                    'v2_avg_costs_bps': sum(t.costs_bps for t in labeled_trades) / len(labeled_trades) if labeled_trades else 0,
                }

                if weights is not None:
                    ess = self.v2_adapter.weighter.get_effective_sample_size(weights)
                    v2_metrics['v2_effective_samples'] = ess
                    v2_metrics['v2_sample_efficiency'] = ess / len(weights) if weights.size > 0 else 0

                logger.info("v2_data_pipeline_complete", **v2_metrics)

                # Optional: Run walk-forward validation
                if self.v2_adapter.config.enable_walk_forward:
                    validation_results = self.v2_adapter.validate_trades(labeled_trades)
                    v2_metrics['v2_validation'] = validation_results

        self._seed_replay_from_memory()

        # 3. Run shadow trading - every possible trade
        ...
        # 4. Update agent with accumulated experience
        if len(trades) > 0:
        ...
        # 5. Analyze aggregate performance
        ...
        # 6. Learn optimal parameters for discovered patterns
        ...
        metrics = {
            ...
            "v2_pipeline_enabled": self.use_v2_pipeline,
            **update_metrics,
            **v2_metrics  # Add V2 metrics
        }
        ...
        logger.info("training_complete_v2", **metrics)
        return metrics
    ...
    def train_on_universe(
        ...
        Same as original but with V2 enhancements per symbol.
        """
        logger.info("universe_training_start_v2", symbols=len(symbols), v2_enabled=self.use_v2_pipeline)
        ...
        if self.use_v2_pipeline:
            v2_total_labels = sum(r.get("v2_total_labels", 0) for r in results)
            v2_profitable_labels = sum(r.get("v2_profitable_labels", 0) for r in results)

            logger.info(
                "universe_training_complete_v2",
                symbols=len(symbols),
                total_trades=total_trades,
                total_wins=total_wins,
                overall_win_rate=total_wins / total_trades if total_trades > 0 else 0.0,
                total_profit_gbp=total_profit,
                v2_total_labels=v2_total_labels,
                v2_profitable_labels=v2_profitable_labels,
                v2_profitable_pct=v2_profitable_labels / v2_total_labels * 100 if v2_total_labels > 0 else 0
            )
        else:
            logger.info(
    ...
    # All helper methods same as original
    ...
    def _run_curriculum_phases(self, symbol: str, historical_data: pl.DataFrame) -> Dict[str, Any]:
        """Run curriculum learning phases."""
    ...
    def _build_curriculum_phases(self, data: pl.DataFrame) -> List[tuple[str, pl.DataFrame]]:
        """Build curriculum learning phases."""
        if data.height < 200:
            return []
