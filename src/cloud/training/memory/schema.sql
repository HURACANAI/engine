...
CREATE TABLE trade_memory (
    ...
    entry_embedding vector(128),     -- Vector embedding for similarity search
...
CREATE TABLE post_exit_tracking (
...
CREATE INDEX idx_trade_memory_entry_ts ON trade_memory(entry_timestamp);
CREATE INDEX idx_trade_memory_winner ON trade_memory(is_winner);
CREATE INDEX idx_trade_memory_regime ON trade_memory(market_regime);
CREATE INDEX idx_trade_memory_embedding ON trade_memory USING ivfflat (entry_embedding vector_cosine_ops);

CREATE INDEX idx_pattern_library_embedding ON pattern_library USING ivfflat (pattern_embedding vector_cosine_ops);
CREATE INDEX idx_pattern_library_win_rate ON pattern_library(win_rate DESC);

CREATE INDEX idx_post_exit_trade ON post_exit_tracking(trade_id);
CREATE INDEX idx_win_analysis_trade ON win_analysis(trade_id);
CREATE INDEX idx_loss_analysis_trade ON loss_analysis(trade_id);
